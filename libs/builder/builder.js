...
Vvveb.Components = {
	...
	render: function (type) {
		...
		if (!(Vvveb.preservePropertySections && section.length)) {
			rightPanel.html('').append(tmpl("vvveb-input-sectioninput", {
				key: "default",
			...
			if (event.target && isElement(event.target)) {
				...
				if (self.isDragging) {
					...
					if (event.originalEvent) self.iconDrag.css({
						'left': event.originalEvent.x + 275 /*left panel width*/ ,
						'top': event.originalEvent.y - 30
					});
...
Vvveb.FileManager = {
	...
	loadPage: function (name) {
		...
		if (name.toLowerCase().indexOf('magic') > -1) {
			var _this = this;
			$.ajax("http://localhost:8080/page-import", {
				success: function (data) {
					console.log(data)
					Vvveb.Builder.loadUrl(_this.pages[name]['url'],
						function () {
